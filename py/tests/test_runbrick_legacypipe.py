...
def rbmain():
    ...
    survey = LegacySurveySim(surveydir)
    ...
    main(args=['--radec', '346.684', '12.791', '--width', '100',
               ...
               '--plots', '--plot-base',os.path.join('out-testcase12','brick-%(brick)s')])
    ...
    main(args=['--radec', '9.1228', '3.3975', '--width', '100',
               ...
               '--plots', '--plot-base',os.path.join('out-testcase9','brick-%(brick)s'), '--stage', 'halos'])
    ...
    # test forced phot??
    '''
    shutil.copy(os.path.join(os.path.dirname(__file__),'testcase9','survey-bricks.fits.gz'), 'out-testcase9b')
    ...
    main(args=['--brick', '1867p255', '--zoom', '2050', '2300', '1150', '1400',
               ...
               '--survey-dir', surveydir,
               '--stage', 'image_coadds',
               '--outdir', 'out-testcase4b', '--plots', '--plot-base',os.path.join('out-testcase4b','brick-%(brick)s')])
    print('Checking for calib file', fn)
    assert(os.path.exists(fn))
    ...
    fn = survey.find_file('tractor', brick='2447p120', output=True)
    print('Checking', fn)
    T = fits_table(fn)
    cat = read_fits_catalog(T)
    ...
    main(args=['--brick', '2447p120', '--zoom', '1020', '1070', '2775', '2815',
               '--no-wise', '--force-all', '--no-write',
               '--survey-dir', surveydir,
               '--outdir', 'out-testcase3', '--plots', '--plot-base',os.path.join('out-testcase3','brick-%(brick)s'),
               '--nblobs', '1'])
